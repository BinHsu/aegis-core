...
| **Phase 4** | Packaging (OCI, Cosign, SLSA L3), progressive delivery, observability, cloud auth | 🚧 **4a/4b/4c/4d/4e substantially complete on aegis-core side**. **4a**: OCI + ECR + frontend S3/CloudFront (ADR-0025/0027). **4b**: Cosign + SLSA L3 + Trivy + govulncheck + gosec + Checkov + kube-score (CodeQL/Semgrep/clang-tidy/kube-bench queued). **4c**: Deployment → Argo Rollouts CRD (ADR-0030), Kyverno R6 audio-ns isolation, post-deploy Playwright nightly, live ACM ARN + Qdrant/Cognito env wiring into engine Rollout. **4d**: `/metrics` Prometheus (:8081, ADR-0033) + OpenTelemetry OTLP tracer with ADR-0005 R4 attribute allowlist + `aegis_gateway_hints_emitted_total` / `aegis_gateway_host_transient_loss_total` domain metrics. Engine OTLP + C-Obs-2 Grafana Cloud 5-CRD still queued. **4e**: `auth.OIDCProvider` (Cognito JWKS) + SPA `react-oidc-context` + tenant_id propagation + nightly integration test against live ldz pool (ADR-0034). Cold-apply awaiting ldz workload-workflow trigger (target before 2026-05-07 Grafana Cloud trial downgrade). |
