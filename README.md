...
| **Phase 4** | Packaging (OCI, Cosign, SLSA L3), progressive delivery, observability | 🚧 4a Slices 1–4 landed: `rules_oci` wiring + Go Gateway distroless image (ADR-0025) with mandatory CI runtime smoke (`docker run --read-only --user 65532:65532` + `/healthz` 200); CycloneDX SBOM via syft (`anchore/sbom-action` SHA-pinned); ECR push via OIDC role on merge to `main` (tag `staging-<git_sha>`); C++ engine image (`engine-staging-<git_sha>`, distroless static, Linux-build-gated, models mount-at-runtime). Frontend image, engine SBOM, Cosign / SLSA / Trivy still queued |
