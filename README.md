...
| **Phase 4** | Packaging (OCI, Cosign, SLSA L3), progressive delivery, observability | 🚧 4a Slices 1–5 landed + 4b mini-slice 1: `rules_oci` wiring + Go Gateway distroless image (ADR-0025) with mandatory CI runtime smoke; CycloneDX SBOM via syft; ECR push via OIDC role on merge to `main`; C++ engine image (distroless static, models mount-at-runtime per ADR-0026); frontend deploy via S3 + CloudFront (ADR-0027); **Cosign keyless signing of gateway + engine images + signed SBOM attestation (ADR-0028)**. SLSA L3, Trivy, Kyverno verify (ldz side), engine SBOM still queued |
