...
Aegis Core does not require a system Go install — the Go 1.24.13 SDK is
...
sibling `aegis-platform-aws` repo per
...
`aegis-platform-aws` must provide"). The migration opens a
`cross-repo/blocking` issue on `aegis-platform-aws` first,
