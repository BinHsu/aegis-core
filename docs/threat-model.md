...
| T3 | Compromised dependency (A5) injects code into production build | Any binary | SBOM, Cosign signing, SLSA provenance, dependency pinning, license scan (§10.1). **Live as of Phase 4a-1**: container base image (`gcr.io/distroless/static-debian12:nonroot`) pinned by SHA256 manifest-list digest in `MODULE.bazel`'s `oci.pull` (ADR-0025); image runs as `nonroot` (uid 65532) under read-only rootfs, validated by mandatory CI smoke step. SBOM (Syft / CycloneDX), Cosign keyless sign, SLSA L3 provenance still queued (Phase 4a-2 / 4b). | Medium — supply chain is never fully solvable; new transitive trust on `gcr.io/distroless` registry availability + image-publisher integrity (digest pinning bounds the latter) |
