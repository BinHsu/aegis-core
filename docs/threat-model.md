...
| T7 | Leaked `AEGIS_CORE_DEPLOY_PAT` lets an attacker push arbitrary K8s manifests to `aegis-core-deploy`, which ArgoCD then reconciles onto the cluster | `aegis-core-deploy` repo / `release-staging-image.yml` `bump-image-tag` job | PAT is **fine-grained**, scoped to the single `aegis-core-deploy` repo with `contents: write` only (no org, no other repos), and expiring; stored as a GitHub Actions repository secret, masked in logs (`::add-mask::`). The deploy repo is unprotected **by design** (ADR-0036 D3) — the compensating control is that the *images* it points at are still Cosign-signed + SLSA-attested (T3), and ldz-side Kyverno verify-image admission (queued) rejects unsigned images regardless of what a tampered manifest references. Defence-in-depth gap: a tampered manifest could still alter non-image fields (resources, env, NetworkPolicy). | Medium — accepted as the deliberate cost of removing the branch-protection fight (ADR-0036 §Consequences); bounded by PAT scope + expiry + image-signature enforcement |
