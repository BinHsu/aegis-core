...
| SBOM emitted + signed per image, machine-readable | Every container image gets a CycloneDX SBOM (Syft via `anchore/sbom-action`, SHA-pinned for reproducibility), attached as a signed Cosign CycloneDX attestation. Gateway + engine both live as of 2026-04-25; frontend is a CDN-served SPA (ADR-0027) so the SBOM-as-OCI-attestation pattern doesn't apply — frontend supply-chain integrity rides on the hermetic pnpm lockfile + GitHub OIDC-scoped S3 push (no SBOM artefact yet). ARCHITECTURE.md §10.1 image-side promise is fully live for both runtime images. | `ADR-0025` §"Sequencing across slices", `.github/workflows/release-staging-image.yml` (`Generate gateway / engine image SBOM` steps), `ARCHITECTURE.md` §10.1 |
...
| Cosign keyless signing — every image cryptographically tied to a workflow run | Phase 4b mini-slice: Sigstore Cosign signs gateway + engine OCI manifests after ECR push, using GitHub OIDC token → Fulcio CA (no long-lived signing key); records every signing event in Rekor public transparency log; gateway CycloneDX SBOM attached as a signed Cosign attestation. Subject claim (`job_workflow_ref` pin) matches the IAM trust policy condition aegis-core already pins per ldz #79 — single source of truth for "what's authorized to sign". Verification side filed cross-repo (ldz #96, Phase 4c Kyverno admission). Cost: $0/month + ~30s/run. | `ADR-0028`, `.github/workflows/release-staging-image.yml` (Cosign steps), ldz #96, Sigstore Public Good infrastructure |
| SLSA L3 provenance + Trivy CVE block — supply-chain trifecta complete | Phase 4b mini-slices 2+3 layered on Cosign: `aquasecurity/trivy-action` SHA-pinned scans every image by `@sha256:` digest (no tag-race) and blocks push on CRITICAL CVEs with available fixes; `actions/attest-build-provenance` SHA-pinned emits SLSA L3 in-toto v1 provenance (Git commit + GitHub runner + workflow path) stored in both the GitHub attestation store AND ECR alongside the image. Kyverno admission policy (ldz #96) gates pod creation on Cosign signature + SLSA provenance — defense-in-depth. The 2024+ canonical supply-chain trifecta (Sigstore + SLSA + Trivy) any security reviewer recognizes. | `ADR-0029`, `.github/workflows/release-staging-image.yml` (Trivy + attest-build-provenance steps), ldz #96 |
