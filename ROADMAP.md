...
**Last Updated**: 2026-04-20 (Phase 4c C-1 foundation manifests landed — `deploy/staging/` holds gateway + engine K8s manifests with ADR-0025 security posture, ADR-0017 headless engine Service, ADR-0006 graceful shutdown timeouts. Bootstrap image tags reference the last known-good staging SHA; C-1.5 picks the tag-update automation pattern (Argo Image Updater vs CI-commits-tag). Probes + model PV + mTLS deferred to C-2 with explicit manifest comments. Earlier today: ADR-0030 (Argo Rollouts over Flagger) + ADR-0031 (cert-manager mTLS, no mesh) + LOCAL posture addendum + ARCH §5/§8 forward refs + Phase 5 "Advanced Operations" placeholders. Also earlier: Phase 4b residual tick — `AEGIS_DEV_AUDIO_DUMP` CI gate landed as Bazel `sh_test` (ADR-0005 R7). Prior 2026-04-19: Phase 4a EXITED; Phase 4b mini-slices 1–3 landed. ADRs 0025 / 0026 v2 + Revision / 0027 / 0028 / 0029 / 0030 / 0031 capture the design pivots.)
...
- [x] **C-1 Foundation manifests** — `deploy/staging/` with Namespace + gateway (Deployment + Service + Ingress + SA) + engine (Deployment + headless Service + SA). ADR-0025 security posture (distroless nonroot + readOnly rootfs + drop ALL caps). ADR-0017 headless Service for engine (gRPC client-side round-robin DNS resolution). ADR-0006 graceful shutdown (30s gateway / 60s engine). Image tags reference the last known-good staging SHA as bootstrap; ArgoCD on ldz consumes the path. Probes + model PV + mTLS all deferred with explicit comments to C-2 / C-5a.
