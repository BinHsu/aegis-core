...
**Last Updated**: 2026-04-20 (Phase 4c C-1 foundation manifests landed — `apps/staging/` (path matched to ldz #54 root ArgoCD Application) holds gateway + engine K8s manifests with ADR-0025 security posture, ADR-0017 headless engine Service, ADR-0006 graceful shutdown timeouts, NetworkPolicy allow-rules layered on ldz's default-deny, IRSA annotation on engine SA. Namespace owned by ldz Terraform (not re-declared here). Bootstrap image tags reference the last known-good staging SHA; C-1.5 picks the tag-update automation pattern (Argo Image Updater vs CI-commits-tag). Probes + model PV + mTLS deferred to C-2 with explicit manifest comments. Earlier today: ADR-0030 (Argo Rollouts over Flagger) + ADR-0031 (cert-manager mTLS, no mesh) + LOCAL posture addendum + ARCH §5/§8 forward refs + Phase 5 "Advanced Operations" placeholders. Also earlier: Phase 4b residual tick — `AEGIS_DEV_AUDIO_DUMP` CI gate landed as Bazel `sh_test` (ADR-0005 R7). Prior 2026-04-19: Phase 4a EXITED; Phase 4b mini-slices 1–3 landed. ADRs 0025 / 0026 v2 + Revision / 0027 / 0028 / 0029 / 0030 / 0031 capture the design pivots.)
...
- [ ] **C-1.5 Image tag update loop** — pick between Argo CD Image Updater (pull-based, auto-commits new tag to this repo on ECR push) OR extending `release-staging-image.yml` with a final `git commit` step that rewrites the manifest's tag in place (push-based GitOps). Current manifest holds a static bootstrap SHA so each release requires one manual edit until this lands. Write ADR-0032 (or a companion section in ADR-0030) documenting the decision.
