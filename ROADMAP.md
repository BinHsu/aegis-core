...
- [ ] **Engine startup manifest validation** (ADR-0026 v2) — engine pre-flight walks the bundled `models/manifest.json`, computes path `<storage>/<model_name>/<sha256>.<ext>` per CAS layout for each `"required": true` entry, HEAD-checks file exists + size matches; trusts content by construction (filename IS the SHA, no recompute). Tolerates "extra" objects from other concurrently-deployed engine versions. Fails loudly with operator-readable diagnostic on missing / wrong-size before starting gRPC server. Today (Slice 4a-4) engine just `mmap`s a single hardcoded path and crashes on miss; honest enough for staging smoke, not acceptable for prod-cut or rolling deploys.
...
- [ ] ~~Lifecycle prune Job for `/models` storage~~ — **DEFERRED** per ADR-0026 v2 §"Pruning deferred". Cost trade is overwhelmingly against pruning at current scale (~$0.05/month per year of accumulation; years before reaching $1/month); operational risk of accidental over-prune outweighs the saved storage. Revisit when accumulated retired-model storage exceeds $10/month.
