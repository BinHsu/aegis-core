...
- [x] **kube-score** manifest best-practice scorer — wired into `ci-baseline.yml` `kube-score` job (PR #82); advisory-only on PR-time per the Phase 4b "see baseline first then flip to hard gate" graduation discipline.
...
- [x] **Checkov** IaC scanner for K8s manifests — wired into `ci-baseline.yml` `checkov-k8s` job (PR #82); advisory-only on PR-time. Complements kube-score from the misconfiguration / policy-as-code angle. **NOT** a secret-scan layer; `gitleaks` + GitHub push protection remain primary per `ARCHITECTURE.md` §10.3 (debrief discussion 2026-04-12).
...
- [x] **C-1 Foundation manifests** — `apps/staging/` (path matches ldz root ArgoCD Application per ldz #54) holds gateway (Deployment + Service + Ingress + SA + NetworkPolicy) + engine (Deployment + headless Service + IRSA-annotated SA + NetworkPolicy). ADR-0025 security posture (distroless nonroot + readOnly rootfs + drop ALL caps). ADR-0017 headless Service for engine (gRPC client-side round-robin DNS resolution). ADR-0006 graceful shutdown (30s gateway / 60s engine). NetworkPolicy allow-rules on top of ldz's default-deny (gateway ← ALB:8080, engine ← gateway:50051, both → AWS APIs:443). Engine SA annotated with pre-provisioned IRSA role `aegis-staging-aegis-engine` (permission policy empty skeleton on ldz side). Image tags reference last known-good staging SHA as bootstrap; ArgoCD on ldz consumes the path. Probes + model PV + mTLS all deferred with explicit comments to C-2 / C-5a.
- [x] **C-1.5 Image tag update loop** — **implemented per [ADR-0032](docs/adr/0032-image-tag-update-automation-deferred.md) (re-decided 2026-05-17)**. `release-staging-image.yml` gains a `bump-image-tag` job: after the images are pushed, it rewrites the `apps/staging/**` image refs with `yq` and opens an auto-merge PR using only the built-in `GITHUB_TOKEN` — no PAT, no controller, because the manifests live in this same repo. The signed-commit wall is cleared via the `createCommitOnBranch` GraphQL API (GitHub signs server-side). Repo prerequisite: enable *Settings → Actions → General → "Allow GitHub Actions to create and approve pull requests"* so the job can open the PR. The bot **cannot** be a ruleset bypass actor on this personal repo (GitHub `422`), so the tag-bump PR runs in **degraded mode** — the repo admin merges it; the `bump-image-tag` step is fail-soft so this never reds the release run (corrected in ADR-0032 + Incident 18). The 2026-04-20 deferral is preserved as "Superseded reasoning" in the ADR — it overestimated setup cost by assuming a cross-repo write-back.
...
- [x] **C-Obs-1 engine + gateway `/metrics` endpoint on :8081** (ADR-0033, consumes ldz #46 observability contract) — prometheus-cpp via Bazel Central Registry for engine, `prometheus/client_golang` for gateway. 4 baseline metrics per service: `{engine|gateway}_up`, engine `model_loaded{model}` / gateway `active_sessions`, `*_rpc_total{method,status}`, `*_rpc_duration_seconds{method}`. gRPC interceptor chain for gateway; RAII instrumentation for engine. `ServiceMonitor` CRs in `apps/staging/aegis-core-{gateway,engine}/` — originally targeted ldz's kube-prometheus-stack; now discovered by **Grafana Alloy** per the 2026-04-21 platform reversal to Grafana Cloud free tier (ldz ADR-022 + ADR-023, PR #123 merged). No aegis-core code change needed for the reversal; the ServiceMonitor CRD shape is identical between kube-prometheus-stack and Alloy. Default-on with `AEGIS_*_METRICS_ADDR=""` env opt-out; `cmd/app_local` disables engine side to avoid LOCAL port collision.
