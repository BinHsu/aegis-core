...
- [x] **kube-score** manifest best-practice scorer — was wired into `ci-baseline.yml` `kube-score` job (PR #82). **Removed from aegis-core CI (2026-05-18, ADR-0036)** when the deploy manifests moved out to `aegis-core_deploy` — there is no `apps/staging/` here for it to scan. Manifest scanning re-establishes in `aegis-core_deploy`'s own CI as a follow-up.
...
- [x] **Checkov** IaC scanner for K8s manifests — was wired into `ci-baseline.yml` `checkov-k8s` job (PR #82). **Removed from aegis-core CI (2026-05-18, ADR-0036)** alongside `kube-score` when the deploy manifests moved out to `aegis-core_deploy`; re-establishes in that repo's CI as a follow-up. **NOT** a secret-scan layer; `gitleaks` + GitHub push protection remain primary per `ARCHITECTURE.md` §10.3 (debrief discussion 2026-04-12).
- [ ] CodeQL, Semgrep, gosec, govulncheck, clang-tidy in CI (ARCH §10.2)
- [ ] ECR push pipeline; ArgoCD polls the deploy manifests — which now live in the `aegis-core_deploy` repo (ADR-0036), not in this repository

### Phase 4c: Progressive Delivery
- [x] **C-1 Foundation manifests** — gateway (Deployment + Service + Ingress + SA + NetworkPolicy) + engine (Deployment + headless Service + IRSA-annotated SA + NetworkPolicy). ADR-0025 security posture (distroless nonroot + readOnly rootfs + drop ALL caps). ADR-0017 headless Service for engine (gRPC client-side round-robin DNS resolution). ADR-0006 graceful shutdown (30s gateway / 60s engine). NetworkPolicy allow-rules on top of ldz's default-deny (gateway ← ALB:8080, engine ← gateway:50051, both → AWS APIs:443). Engine SA annotated with pre-provisioned IRSA role `aegis-staging-aegis-engine` (permission policy empty skeleton on ldz side). Probes + model PV + mTLS all deferred with explicit comments to C-2 / C-5a. **Manifest location (per [ADR-0036](docs/adr/0036-deploy-topology-platform-tier.md), 2026-05-18):** these manifests originally lived in this repo at `apps/staging/`; they have since been **moved out** to the separate `aegis-core_deploy` repo (`k8s/base/aegis-core-{gateway,engine,policies}/`), a dedicated unprotected GitOps deploy repo. `apps/staging/` no longer exists in aegis-core. ArgoCD watches `aegis-core_deploy`.
- [x] **C-1.5 Image tag update loop** — **implemented; mechanism now per [ADR-0036](docs/adr/0036-deploy-topology-platform-tier.md) (2026-05-18), superseding [ADR-0032](docs/adr/0032-image-tag-update-automation-deferred.md)'s same-repo design**. `release-staging-image.yml`'s `bump-image-tag` job: after the images are pushed, it rewrites the image tag in the deploy manifests — now in the separate `aegis-core_deploy` repo, not `apps/staging/` here — and **pushes the change directly to that repo's `main`**. `aegis-core_deploy` has no branch protection by design, so this is a plain `git push` with no PR, no `createCommitOnBranch` signing, no ruleset bypass actor — the whole branch-protection fight ADR-0032 wrestled with (degraded mode, the `422`) is gone. Auth is a fine-grained PAT in the `AEGIS_CORE_DEPLOY_PAT` secret (scoped to `aegis-core_deploy`, `contents: write`); the job is fail-soft if the secret is unset. The manifests use the SHORT image form `aegis-core:<tag>` (registry de-hardcoded into the kustomize overlay), so the bump rewrites only the tag. ArgoCD reconciles `aegis-core_deploy`. **Forward-compatibility (multi-region template, 2026-05-21):** a `Wait for ECR cross-region replication` step in `push-staging-image` gates `bump-image-tag` on each destination region's ECR carrying the new tag — driven by `vars.ECR_DESTINATION_REGIONS` (default empty → no-op today; when multi-region activates, the bump cannot race ahead of replication).
...
- [x] **C-Obs-1 engine + gateway `/metrics` endpoint on :8081** (ADR-0033, consumes ldz #46 observability contract) — prometheus-cpp via Bazel Central Registry for engine, `prometheus/client_golang` for gateway. 4 baseline metrics per service: `{engine|gateway}_up`, engine `model_loaded{model}` / gateway `active_sessions`, `*_rpc_total{method,status}`, `*_rpc_duration_seconds{method}`. gRPC interceptor chain for gateway; RAII instrumentation for engine. `ServiceMonitor` CRs ship per service (now in `aegis-core_deploy` at `k8s/base/aegis-core-{gateway,engine}/` per ADR-0036) — originally targeted ldz's kube-prometheus-stack; now discovered by **Grafana Alloy** per the 2026-04-21 platform reversal to Grafana Cloud free tier (ldz ADR-022 + ADR-023, PR #123 merged). No aegis-core code change needed for the reversal; the ServiceMonitor CRD shape is identical between kube-prometheus-stack and Alloy. Default-on with `AEGIS_*_METRICS_ADDR=""` env opt-out; `cmd/app_local` disables engine side to avoid LOCAL port collision.
