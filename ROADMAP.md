...
- [ ] **Engine startup manifest validation + SHA verify** (ADR-0026 §Revision 2026-04-19) — engine pre-flight walks the bundled `models/manifest.json`, computes path `<storage>/<model_name>/<sha256>.<ext>` per CAS layout for each `"required": true` entry, **stat-checks (exists + size) AND SHA-256-recomputes** the on-disk file (~10s/GB tax for defense-in-depth against bit rot / partial-write / silent corruption — initial trust-by-construction optimization rejected). Tolerates "extra" objects from other concurrently-deployed engine versions. Fails loudly with operator-readable diagnostic on missing / wrong-size / wrong-SHA before starting gRPC server. Today (Slice 4a-4) engine just `mmap`s a single hardcoded path and crashes on miss; honest enough for staging smoke, not acceptable for prod-cut or rolling deploys.
