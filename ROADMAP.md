...
- [x] **C-Obs-1 engine + gateway `/metrics` endpoint on :8081** (ADR-0033, consumes ldz #46 observability contract) — prometheus-cpp via Bazel Central Registry for engine, `prometheus/client_golang` for gateway. 4 baseline metrics per service: `{engine|gateway}_up`, engine `model_loaded{model}` / gateway `active_sessions`, `*_rpc_total{method,status}`, `*_rpc_duration_seconds{method}`. gRPC interceptor chain for gateway; RAII instrumentation for engine. `ServiceMonitor` CRs in `apps/staging/aegis-{gateway,engine}/` — originally targeted ldz's kube-prometheus-stack; now discovered by **Grafana Alloy** per the 2026-04-21 platform reversal to Grafana Cloud free tier (ldz ADR-022 + ADR-023, PR #123 merged). No aegis-core code change needed for the reversal; the ServiceMonitor CRD shape is identical between kube-prometheus-stack and Alloy. Default-on with `AEGIS_*_METRICS_ADDR=""` env opt-out; `cmd/app_local` disables engine side to avoid LOCAL port collision.
