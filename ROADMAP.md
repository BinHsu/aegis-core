...
- [x] **C-1.5 Image tag update loop** — **implemented per [ADR-0032](docs/adr/0032-image-tag-update-automation-deferred.md) (re-decided 2026-05-17)**. `release-staging-image.yml` gains a `bump-image-tag` job: after the images are pushed, it rewrites the `apps/staging/**` image refs with `yq` and opens an auto-merge PR using only the built-in `GITHUB_TOKEN` — no PAT, no controller, because the manifests live in this same repo. The signed-commit wall is cleared via the `createCommitOnBranch` GraphQL API (GitHub signs server-side). One-time ops prerequisite: add `github-actions[bot]` to the `main` ruleset's `bypass_actors` with `bypass_mode: pull_request` (documented in the ADR). The 2026-04-20 deferral is preserved as "Superseded reasoning" in the ADR — it overestimated setup cost by assuming a cross-repo write-back.
