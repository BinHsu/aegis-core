...
- [ ] **Engine startup manifest validation + multi-version-aware load** (ADR-0026) — engine pre-flight walks the bundled `models/manifest.json`, picks filenames per `"required": true` entries (filename encodes the model version), verifies each file is present at the expected path under `/models` with the expected SHA256, tolerates "extra" files belonging to other concurrently-deployed engine versions, fails loudly with operator-readable error BEFORE starting the gRPC server. Today (Slice 4a-4) engine just `mmap`s a single hardcoded path and crashes on miss; honest enough for staging smoke, not acceptable for prod-cut or rolling deploys. Surfaced in `packaging/engine/BUILD.bazel` "HONEST GAP" comment to keep the pre-flight visible until the work lands.
- [ ] **Lifecycle prune Job for `/models` storage** (ADR-0026, ldz #82-follow-on) — periodic Kubernetes Job lists all engine pods, reads each pod's bundled `manifest.json`, computes the union of referenced filenames, deletes any storage file NOT in that union. Conservative pruning (never delete a referenced file; reference list is source of truth, not "latest manifest"). aegis-core provides the manifest-walk CLI; ldz authors the Job manifest. Without this Job, storage grows monotonically — manageable for demo horizon, blocker for prod.
