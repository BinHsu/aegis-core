...
- [~] Bazel `rules_oci`: package C++ engine, Go GW, and frontend into Distroless OCI images
  ...
  - [x] Slice 5 — Frontend deploy via S3 + CloudFront (ADR-0027). NOT an OCI image — original ROADMAP entry was a category error; static SPA bundle goes on a CDN, not a container. Split subdomains: `aegis-app.staging.binhsu.org` (CloudFront → S3) for SPA, `aegis-api.staging.binhsu.org` (ALB → gateway pod, Phase 4c) for API. New `release-staging-frontend.yml` workflow on `push: branches: [main]` builds via hermetic Node, syncs to S3, invalidates CloudFront. New OIDC role `github-actions-aegis-core-frontend` provisioned by ldz per [#91](https://github.com/BinHsu/aegis-aws-landing-zone/issues/91). Gateway CORS allowlist (`AEGIS_ALLOWED_ORIGINS` env var, new `gateway_go/internal/cors/` package) closes Phase 2 Known Gap; tested. Vite `VITE_AEGIS_GATEWAY_ENDPOINT` env var injects API URL at build time.
