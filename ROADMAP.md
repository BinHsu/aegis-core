...
- [ ] **Cloud RAG backend wiring** — `apps/staging/aegis-engine/deployment.yaml:61-66` today sets only `AEGIS_MODEL_PATH` (and its value `/models/ggml-tiny.en.bin` is a pre-ADR-0026 file-path shape, not the CAS root shape the current engine expects — a drift of its own). `QDRANT_URL` is unset so the engine's RAG path fail-closes with `QDRANT_URL unset (RAG hints disabled)` even after the image runs, and there is no in-cluster Qdrant / managed endpoint / ExternalName Service for it to reach anyway. Three coupled decisions blocking the Cloud min-bar (speech → ASR → RAG → hint): (a) Qdrant backend infra — in-cluster StatefulSet vs AWS OpenSearch Serverless vs Qdrant Cloud free tier — tracked in [ldz #130](https://github.com/BinHsu/aegis-aws-landing-zone/issues/130); (b) corpus seed loop on Cloud — one-shot K8s Job running `engine seed --target=cloud` (the subcommand already handles `--target=cloud`, reading `QDRANT_URL` + `QDRANT_API_KEY` from env); (c) Secret/ConfigMap + ExternalSecret wiring of `QDRANT_URL` + `QDRANT_API_KEY` into the Deployment env. Blocked on (a); (b)+(c) can scaffold as a single PR once (a) names the backend. LAN exercises the whole retriever end-to-end today via `CONTRIBUTING.md` §LAN smoke, so the code path is proven; Cloud is purely an infra-wiring gap.
