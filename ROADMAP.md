...
- [x] **C-1.5 Image tag update loop** — **deferred per [ADR-0032](docs/adr/0032-image-tag-update-automation-deferred.md)**. Branch protection (signed + CODEOWNERS-reviewed commits, admin-only bypass) makes direct automation hostile to the policy; at current release cadence (~3/week) the annual time save (~20 min) is smaller than the setup cost (~4h) of any automated alternative. During active Phase development, slice PRs naturally absorb the SHA bump. ADR documents 5 triggers to reopen (release cadence > manifest-change cadence, new contributor joins, multi-region lands, release-defect traced to forgotten bump, or bypass_actors policy changes).
