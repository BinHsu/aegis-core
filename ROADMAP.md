...
- [ ] ~~Lifecycle prune Job for `/models` storage~~ — **DEFERRED** per ADR-0026 v2 §"Pruning deferred". Cost trade is overwhelmingly against pruning at current scale (~$0.05/month per year of accumulation; years before reaching $1/month); operational risk of accidental over-prune outweighs the saved storage. Revisit when accumulated retired-model storage exceeds $10/month. **When pruning eventually activates, complexity stays simple (time-based / latest-N retention) — pod startup SHA verify per ADR-0026 §Revision 2026-04-19 is the structural safety net for false-prune (loud pod failure → operator decides) instead of conservative reference-counted gymnastics.**
